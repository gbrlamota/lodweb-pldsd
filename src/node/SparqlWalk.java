...
public class SparqlWalk {	
	...
	public static void main(String[] args) {
      ...
      //System.out.println(getDescription2("http://dbpedia.org/resource/Finding_Nemo"));
      ...
      System.out.println(getIndirectDistinctInconmingLinksBetween2Resources("http://dbpedia.org/resource/The_Terminator","http://dbpedia.org/resource/Tarzan"));
      System.out.println(getIndirectDistinctOutgoingLinksBetween2Resources("http://dbpedia.org/resource/Return_of_the_Jedi","http://dbpedia.org/resource/Aliens_(film)"));
	 ...
	 * @return
	 */
	public static String getDescription2(String subjectURI) {
		...
		" SELECT distinct ?x  WHERE { <"+ subjectURI + ">  <http://dbpedia.org/ontology/abstract> ?x . FILTER (langMatches(lang(?x),'en'))}";
	 ...
	 * @return
	 */
	public static int getCountLiteralByUri(String subjectURI) {
		...
		String queryString = "" + Sparql.addService(USING_GRAPH, serviceURI) + 
				"SELECT (count (distinct ?p1) as ?literals)  WHERE {" 
				...
				//+ " FILTER(isLiteral(?r2)) .  "
				//+ " FILTER (isNumeric(?r2)) ."
				+ "FILTER ( datatype(?r2) = <http://dbpedia.org/datatype/usDollar>  "
				+ "|| datatype(?r2) = <http://dbpedia.org/datatype/double> "
				+ "|| <http://dbpedia:datatype/minute> "
				...
				+ " FILTER(?p1 != <http://dbpedia.org/ontology/flag>) . "
				+ " FILTER(?p1 != <http://dbpedia.org/property/imageFlag>). "
				+ " FILTER(?p1 != <http://dbpedia.org/ontology/wikiPageID>) . "
				+ "FILTER(?p1 != <http://dbpedia.org/ontology/wikiPageRevisionID>) . "
				+ "FILTER(?p1 != <http://dbpedia.org/ontology/wikiPageWikiLink>). "
				+ "FILTER(?p1 != <http://dbpedia.org/ontology/wikiPageRedirects>) . "
				+ "FILTER(?p1 != <http://dbpedia.org/ontology/wikiPageDisambiguates>) ."
				+ "FILTER(?p1 != <http://purl.org/linguistics/gold/hypernym>) . "
				+ "}}"
	...
	public static Resource getMostSpecificSubclasseOfDbpediaResource(String resourceURI) {
		...
		queryString.append("SELECT * WHERE { <");
		queryString.append(resourceURI);
		queryString.append("> a ?c1 ; a ?c2 . ?c1 rdfs:subClassOf ?c2. FILTER(STRSTARTS(STR(?c2), 'http://dbpedia.org/ontology')) }");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));

		Query query = QueryFactory.create(Sparql.addPrefix().concat(queryString.toString()));
	...
	public static List<Resource> getDbpediaOntologyDatatypeProperties(String ontologyURI)
	{
		...
		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append(" select distinct ?property  ");
		queryString.append(" WHERE { ?property <http://www.w3.org/2000/01/rdf-schema#domain> <");
		queryString.append(ontologyURI);
		queryString.append("> .");
		queryString.append(" ?property <http://www.w3.org/2000/01/rdf-schema#range> ?r . ?r rdf:type rdfs:Datatype . }}} ");
    ...
    public static List<Resource> getDatatypePropertiesFromResourceTypes(String resourceURI)
    {
        List<Resource> resources = new ArrayList<Resource>();
        // String serviceURI = "http://dbpedia-live.openlinksw.com/sparql";

        String queryString = Sparql.addService(USING_GRAPH, serviceURI) + " select distinct ?property "
                + " WHERE { <"+resourceURI+"> a ?class. ?property <http://www.w3.org/2000/01/rdf-schema#domain> ?class ."
	 ...
	 * @return
	 */
	public static int getCountDirectLinkFrom2ResourcesAndLink(String uri1, String uri2, String property) {

		int totalDirectLinksBetween2ResourcesandProperty = 0;

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));				
		queryString.append(" SELECT (count (?p1) as ?x) WHERE { values (?r1 ?p1 ?r2) {( <" + uri1+ "> <" + property+ ">  <" + uri2+ "> )} " + ". ?r1 ?p1 ?r2 . FILTER (?r1 != ?r2)}"); 
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
		...
		try (QueryExecution qexec = QueryExecutionFactory.create(query, model)) {
			...
			try {
				...
				for (; rs.hasNext();) {
					...
					RDFNode x = rb.get("x");
					totalDirectLinksBetween2ResourcesandProperty = (int) x.asLiteral().getValue();
				}
			} finally {
				qexec.close();
			}
			return totalDirectLinksBetween2ResourcesandProperty;
	 ...
	 * @return
	 */
	public static List<SimpleTriple> getDBpediaObjectsTripletBySubjectURI(String subjectURI) {
		...
		// + "SELECT *"
				+ "WHERE { values (?r1) { ( <" + subjectURI + ">   )} . " + "?r1 ?p1 ?r2 .  "
		//ignores every no dbpedia resource object
				+ " FILTER(STRSTARTS(STR(?r2), " + quotes + "http://dbpedia.org/resource/" + quotes + ")) . "
		//ignores unuseful properties and literal/datatype objects
				+ Sparql.addFilter(null, "p1", "r2")
	 ...
	 * @return
	 */
	public static List<SimpleTriple> getDBpediaObjectsTripletByObjectURI(String objectURI) {

		List<SimpleTriple> triples = new ArrayList<SimpleTriple>();

		String queryString = "" + Sparql.addService(USING_GRAPH, serviceURI) + "SELECT ?r1 ?p1 "
		// + "SELECT *"
				+ "WHERE { values (?r2) { ( <" + objectURI + ">   )} . " + "?r1 ?p1 ?r2 .  "
		//ignores every no dbpedia resource object
				+ " FILTER(STRSTARTS(STR(?r1), " + quotes + "http://dbpedia.org/resource/" + quotes + ")) . "
		//ignores unuseful properties and literal/datatype objects
				+ Sparql.addFilter("r1", "p1", null)
		...
		try (QueryExecution qexec = QueryExecutionFactory.create(query, m)) {
			...
			try {
				...
				for (; rs.hasNext();) {
					...
					RDFNode p = rb.get("p1");
					if (x.isResource() && p.isResource()) {
						//troca as posições do sujeito com o objeto!
						SimpleTriple simpleTriple = new SimpleTriple(objectURI, p.asResource().getURI(),
								x.asResource().getURI());
						triples.add(simpleTriple);
					}
				}
	 ...
	 * @return
	 */
	public static List<SimpleTriple> getDBpediaObjectTripletByPropertyURIBySubjectList(String propertyURI, String subjectList) {

		List<SimpleTriple> triples = new ArrayList<SimpleTriple>();

		String queryString = "" + Sparql.addService(USING_GRAPH, serviceURI) + "SELECT ?r1 ?r2 "
		// + "SELECT *"
				+ "WHERE { values (?p1) { ( <" + propertyURI + ">   )} . " + "?r1 ?p1 ?r2 .  "
				+ " FILTER(?r1 IN ( "  + subjectList +  ")) . "
				//+ Sparql.addFilter("r1", null, null)
		...
		try (QueryExecution qexec = QueryExecutionFactory.create(query, m)) {
			...
			try {
				...
				for (; rs.hasNext();) {
					...
					RDFNode p = rb.get("r2");
					if (x.isResource() && p.isResource()) {
						SimpleTriple simpleTriple = new SimpleTriple(x.asResource().getURI(), propertyURI, 
								p.asResource().getURI());
						triples.add(simpleTriple);
					}
				}
...
/*		if (!IConstants.FILTER_CATEGORY) {
	...
	public static List<Resource> getDataTypePropertiesSharedByResources(String uri1, String uri2) {
		...
		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT distinct ?p1 ");
		queryString.append("WHERE { values (?r1 ?r2) { ( <");
		queryString.append(uri1);
		queryString.append("> <");
		queryString.append(uri2);
        queryString.append(">   )}. ?r1 ?p1 ?x. ?r2 ?p1 ?y. FILTER(isLiteral(?x)). FILTER(isLiteral(?y)). ");
        queryString.append(Sparql.addFilter(null, "p1", null));
        queryString.append("}");
        queryString.append(Sparql.addServiceClosing(USING_GRAPH));
	...
	public static List<Resource> getObjectPropertiesSharedByResources(String uri1, String uri2) {
		...
		Sparql.addService(USING_GRAPH, serviceURI) 
		+ "SELECT distinct ?p1 "
		+ "WHERE { values (?r1 ?r2) { ( <" + uri1 + "> <" + uri2 + ">   )} . "
		+ "?r1 ?p1 ?x .  "
		+ "?r2 ?p1 ?y .  "
		+ Sparql.addFilter(null, null, "x")
		+ Sparql.addFilter(null, null, "y")
		+ "}"
		+ Sparql.addServiceClosing(USING_GRAPH);
	...
	public static List<Literal> getLiteralByResourceAndProperty(String uri1, String property) {
		...
		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT distinct ?r2 WHERE { values (?r1 ?p1) { ( <");
		queryString.append(uri1);
		queryString.append("> <");
		queryString.append(property);
		queryString.append(">   )} . ?r1 ?p1 ?r2 .");
		queryString.append("FILTER(isLiteral(?r2)) ."); 
		queryString.append(Sparql.addLangFilter(null, null, "r2"));
		queryString.append("}");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
	...
	public static double getExtremeValueOfNumericProperty(String propertyUri, String type, boolean ascendingOrder)
	{
		List<Literal> literals = new LinkedList<Literal>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT (xsd:double( xsd:string(?v) )  as ?value) WHERE {?r <");
		queryString.append(propertyUri);
		queryString.append("> ?v.");
		queryString.append("FILTER (");
		queryString.append(type != "" ?  "datatype(?v) = <" + type + ">" : "isNumeric(?v)");
		queryString.append(").");
		if (ascendingOrder)
			queryString.append("} ORDER BY ASC(?value)");
		else
			queryString.append("} ORDER BY DESC(?value)");
		...
		try (QueryExecution qexec = QueryExecutionFactory.create(query, m))
		{
			try
			{
				...
				{
					QuerySolution rb = rs.nextSolution();
					RDFNode x = rb.get("value");
					if (x.isLiteral()) {
						literals.add(x.asLiteral());
					}
				}
			}
			finally
		...
		return literals.get(0).getDouble();
	}
	
	public static double getVarianceOfNumericProperty(String propertyUri, String type)
	{
		List<Literal> literals = new LinkedList<Literal>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT ( ( SUM( (?value - ?avg) * (?value - ?avg) ) / (count(*)) ) as ?variance ) WHERE  {?r <");
		queryString.append(propertyUri);
		queryString.append("> ?v.");
		queryString.append("FILTER (");
		queryString.append(type != "" ?  "datatype(?v) = <" + type + ">" : "isNumeric(?v)");
		queryString.append("). BIND (xsd:double( xsd:string(?v) )  as ?value).");
		queryString.append("{SELECT (avg(?value) as ?avg) WHERE {?r <");
		queryString.append(propertyUri);
		queryString.append("> ?v.");
		queryString.append("FILTER (");
		queryString.append(type != "" ?  "datatype(?v) = <" + type + ">" : "isNumeric(?v)");
	...
	public static double getExtremeValueOfDateProperty(String propertyUri, boolean ascendingOrder)
	{
		int year = 0, month = 0, day = 0;

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT ?value (YEAR(?value) as ?year) (MONTH(?value) as ?month) (DAY(?value) as ?day) WHERE {?r <");
		queryString.append(propertyUri);
		queryString.append("> ?v.");
		//queryString.append("FILTER (datatype(?v) IN (xsd:date, xsd:gDay, xsd:gMonth, xsd:gMonthDay, xsd:gYear, xsd:gYearMonth)).");
		queryString.append("FILTER (!regex(?v,'-2-29$')).");
		queryString.append("FILTER (!regex(?v,'-2-30$')).");
		queryString.append("FILTER (!regex(?v,'-2-31$')).");
		queryString.append("FILTER (!regex(?v,'-4-31$')).");
		queryString.append("FILTER (!regex(?v,'-6-31$')).");
		queryString.append("FILTER (!regex(?v,'-9-31$')).");
		queryString.append("FILTER (!regex(?v,'-11-31$')).");
		queryString.append("FILTER (!regex(?v,'-0$')).");
		queryString.append("FILTER (!regex(?v,'^--')).");
		queryString.append("FILTER (!regex(?v,'^0-')).");
		queryString.append("FILTER (!regex(?v,'^00-')).");
		queryString.append("FILTER (!regex(?v,'^000-')).");
		queryString.append("FILTER (!regex(?v,'^0000-')).");
		queryString.append("FILTER (!regex(?v,'1582-10-12')).");
		queryString.append("FILTER (!regex(?v,'1582-10-13')).");
		queryString.append("FILTER (!regex(?v,'1582-10-14')).");
		queryString.append("FILTER (!isNumeric(?v)).");
		queryString.append("FILTER (!regex(str(?v),'^http')).");
		queryString.append("BIND (bif:stringdate(str(?v)) as ?value).");
		if (ascendingOrder)
			queryString.append(" } ORDER BY ASC(?year) ASC(?month) ASC(?day)");
		else
			queryString.append(" } ORDER BY DESC(?year) DESC(?month) DESC(?day)");
		queryString.append(" LIMIT 1");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
		...
		try (QueryExecution qexec = QueryExecutionFactory.create(query, m))
		{
			try
			{
				...
				{
					QuerySolution rb = rs.nextSolution();
					RDFNode x = rb.get("year");
					if (x.isLiteral()) {
						year = x.asLiteral().getInt();
					}
					x = rb.get("month");
					if (x.isLiteral()) {
						month = Math.min(12, x.asLiteral().getInt());
					}
					x = rb.get("day");
					if (x.isLiteral()) {
						switch (month)
						{
							case 2:
								day = Math.min(28, x.asLiteral().getInt());
								break;
							case 1:
							case 3:
							case 5:
							...
							case 10:
							case 12:
								day = Math.min(31, x.asLiteral().getInt());
								break;
							default:
								day = Math.min(30, x.asLiteral().getInt());
								break;
						}
					}
				}
		...
		return year < 0 ? ((year*-1d) * 10000d + month * 100d + day) * -1d : year * 10000d + month * 100d + day;
	}
	
	public static List<String> getUrisBySubject(String subjectUri)
	{
		List<String> uris = new LinkedList<String>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("select ?resource where {?resource dct:subject <"+subjectUri+">}");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
	...
	public static Resource getMostSpecificCommonSuperclassDbpedia(String resourceUri1, String resourceUri2)
	{
		return getMostSpecificCommonSuperclassDbpedia(resourceUri2, resourceUri1, false);
	}
	
	public static Resource getMostSpecificCommonSuperclassDbpedia(String resourceUri1, String resourceUri2, Boolean finalTry)
	{
		List<Resource> resources = new ArrayList<Resource>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT ?lcs WHERE { <");
		queryString.append(resourceUri1);
		queryString.append("> rdf:type/rdfs:subClassOf* ?lcs . <");
		queryString.append(resourceUri2);
		queryString.append("> rdf:type/rdfs:subClassOf* ?lcs . FILTER NOT EXISTS { <");
		queryString.append(resourceUri1);
		queryString.append("> rdf:type/rdfs:subClassOf* ?sublcs . <");
		queryString.append(resourceUri2);
		queryString.append(">  rdf:type/rdfs:subClassOf* ?sublcs . ?sublcs rdfs:subClassOf ?lcs . FILTER strstarts(str(?sublcs), 'http://dbpedia.org/ontology') } FILTER(STRSTARTS(STR(?lcs), 'http://dbpedia.org/ontology')) }");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
		...
		{
			if (!finalTry)
				return getMostSpecificCommonSuperclassDbpedia(resourceUri2, resourceUri1, true);
			else
				throw ex;
		}
		...
		if (resources.size() == 0)
		{
			resources.add(getMostSpecificCommonSuperclass(resourceUri1, resourceUri2));
		}

		return resources.get(0);
	}

	public static Resource getMostSpecificCommonSuperclass(String resourceUri1, String resourceUri2)
	{
		List<Resource> resources = new ArrayList<Resource>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT ?lcs WHERE { ?lcs ^(rdf:type/rdfs:subClassOf*) <");
		queryString.append(resourceUri1);
		queryString.append(">, <");
		queryString.append(resourceUri2);
		queryString.append("> . FILTER NOT EXISTS { ?sublcs ^(rdf:type/rdfs:subClassOf*) <");
		queryString.append(resourceUri1);
		queryString.append("> , <");
		queryString.append(resourceUri2);
		queryString.append("> ; rdfs:subClassOf ?lcs . }}");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
	...
	public static Resource getRootClass(String classUri)
	{
		List<Resource> resources = new ArrayList<Resource>();

		StringBuilder queryString = new StringBuilder();
		queryString.append(Sparql.addService(USING_GRAPH, serviceURI));
		queryString.append("SELECT DISTINCT ?root WHERE { <");
		queryString.append(classUri);
		queryString.append("> rdfs:subClassOf ?mid . ?mid rdfs:subClassOf* ?root . FILTER NOT EXISTS {?root rdfs:subClassOf ?superroot}}");
		queryString.append(Sparql.addServiceClosing(USING_GRAPH));
		...
		for (Resource resource : resources)
		{
			if(resource.getURI().contains("/owl#Thing"))
				return resource;
		}
