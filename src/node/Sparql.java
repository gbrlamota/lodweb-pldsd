...
public class Sparql {
	...
	public static String addPrefix() {
		...
		prefix.append("PREFIX dct: <http://purl.org/dc/terms/>"); // adicionado por Diego Santana
	...
	public static String addFilter(String subject, String property, String object) {
		...
		if (property != null) {
			property2.append(" FILTER(?"+ property + " != <http://dbpedia.org/ontology/wikiPageID>) . "
					+ "FILTER(?"+ property + " != <http://dbpedia.org/ontology/wikiPageRevisionID>) . "
							+ "FILTER(?"+ property + " != <http://dbpedia.org/ontology/wikiPageWikiLink>). "
									+ "FILTER(?"+ property + " != <http://dbpedia.org/ontology/wikiPageRedirects>) .  "
											+ "FILTER(?"+ property + " != <http://dbpedia.org/ontology/wikiPageDisambiguates>) . "
													+ "FILTER(?"+ property + " != <http://purl.org/linguistics/gold/hypernym>) . ");
		}
		if (object != null) {
