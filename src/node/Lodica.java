...
/*								if (!new File("sql//CNN_CUT_TEST.csv").exists()) {
	...
	private void startTOPNExperiment() throws Exception {
		...
		//fred
		//int LIKED_ITEMS = 15;
		int TESTED_USERS = 100;
		Set<Integer> ids = getDatabaseConnection().getRandomUserIdsForLikedMoviesMusicsBooks(TESTED_USERS);
		//Set<Integer> ids = getDatabaseConnection().getUserIdsWithLikedItemsByRange(LIKED_ITEMS-1,LIKED_ITEMS+1);
		//Set<Integer> ids = getDatabaseConnection().getUserIdsWithLikedItemsByRange(49,51);
		List<Integer> idsFinal = new ArrayList<Integer>(ids).subList(0,Math.min(ids.size(), TESTED_USERS));
...
/*			if (getDatabaseConnection().existCompleteEvaluationByUserId(Lodica.userId)) {
			...
			int totalCompared = getDatabaseConnection().getTotalEvaluationsDoneByUserSoFar(IConstants.SIMILARITY_METHOD_IN_USE,userId);
...
/*		if(IConstants.SIMILARITY_METHOD_IN_USE.equals(IConstants.ABSTRACT_SIMILARITY)){
	...
	private List<NodePrediction> classify() {
		...
		for (Node nodeTest : testNodes) {
			//NodeUtil.print("nodeUnderEvaluation "+nodeUnderEvaluation.getURI());
			//NodeUtil.print("nodeTest "+nodeTest.getURI());
...
/*			if (!IConstants.USE_ICA && !nodeUnderEvaluation.getURI().equals(nodeTest.getURI())) {
			...
			if ((IConstants.USE_ICA && icaIteration==1) || !IConstants.USE_ICA) {
				GraphPrediction graphPrediction = new GraphPrediction(nodeUnderEvaluation.getURI(),nodeTest.getURI(),originalCandidate,DBFunctions.checkSimilarityMethod(IConstants.USE_ICA),userId.intValue(),cnns.size(),NodeUtil.getLabeledNodes(cnns).size(),labeledNeighbours.size());
				getDatabaseConnection().insertOrUpdateGraphPrediction(graphPrediction);					
			}
				
			//System.out.println("labeledNeighbours size "+labeledNeighbours.size());
...
/*			if(isEvaluation && resources.size()<IConstants.MAX_CANDIDATES){
			...
			if (IConstants.MAX_CANDIDATES!=null && resources.size()>IConstants.MAX_CANDIDATES) {
...
/*			//resources = new ArrayList<Resource>(new HashSet<Resource>(resources));
			...
			NodeUtil.print("Huston we have a problem "+e.getMessage());
			return new HashSet<Node>();
		}
